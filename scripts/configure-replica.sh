...
if [ "$DRY_RUN" = true ]; then
    ...
    log_dry_run "  su -m postgres -c \"repmgr -h $PRIMARY_PGHOST -p $PRIMARY_PGPORT -d repmgr -U repmgr -f $REPMGR_CONF standby clone --dry-run\""
    log_dry_run ""
...
EOF
    ...
    if su -m postgres -c \
        "repmgr -h $PRIMARY_PGHOST -p $PRIMARY_PGPORT -d repmgr -U repmgr -f $REPMGR_CONF standby clone --dry-run"; then
